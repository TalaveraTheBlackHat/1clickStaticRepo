 ...
 $(document).on('click','div', function(){
	var divSelect=$(this).prop('class');
	var idSelect=$(this).prop('id');


  if(idSelect.length > 7){
    document.getElementById("product-image").src='../'+idSelect+'';
    document.getElementById("overlay").style='background: url("../'+idSelect+'")';
  }
});
});
...
document.getElementById("overlay").style='background: url("../<%= nroImagen[0] %>")';
    //Initial References
...
imageContainer.addEventListener(events[deviceType].move, (e) => {
  ...
  if (
    imageWidth - (x - imageContainer.offsetLeft) < 30 || 
    x - imageContainer.offsetLeft < 30 
    ||
    imageHeight - (y - imageContainer.offsetTop) < -5 ||
    y - imageContainer.offsetTop < 90
  ) {
    hideElement();
  } else {
  ...
  var posX = ((x - imageContainer.offsetLeft) / imageWidth).toFixed(8) * 100;
  var posY = ((y - imageContainer.offsetTop) / imageHeight).toFixed(8) * 100;
